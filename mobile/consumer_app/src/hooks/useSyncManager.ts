 ...
 * retrying each queued report that is due (not still inside its backoff
 * window). A non-network ApiError marks the item 'failed' after
 * MAX_ATTEMPTS; a 409 CONFLICT is terminal immediately — retrying it
 * would only conflict again, so it never counts toward or respects the
 * attempt bound, it just needs the user's attention.
 *
