...
const ACCESS_KEY = 'ecotrace_collector_access_token';
const REFRESH_KEY = 'ecotrace_collector_refresh_token';
...
describe('secureStorage — web (localStorage fallback, CHANGE-007)', () => {
  const originalOS = Platform.OS;
  ...
  it('never calls expo-secure-store on web (the CHANGE-007 regression)', async () => {
    await secureStorage.setTokens('a', 'b');
