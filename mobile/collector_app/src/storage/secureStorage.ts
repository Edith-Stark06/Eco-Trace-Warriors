 ...
 * `getValueWithKeyAsync is not a function` (CHANGE-007). Falls back to the
 * browser's `localStorage`, which does NOT provide the same security
 * properties as a native secure enclave/keystore: it is plain, unencrypted,
 * per-origin storage readable by any script running on the page (e.g. an
 * XSS vector) and by browser devtools. This is a deliberate, web-only
 * trade-off — native platforms are unaffected and unchanged.
 */
